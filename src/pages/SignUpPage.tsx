...
export const SignUpPage: React.FC<SignUpPageProps> = ({ onNavigate, prefilledReferralCode = '' }) => {
    ...
    debounceTimerRef.current = setTimeout(async () => {
      try {
        ...
        if (res.valid) {
          ...
          setSponsorId(res.sponsor_id || null);
          setSponsorError(null);
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Generate clean unique referral code if table needs manual upsert
      const generatedRefCode = `MO${Math.floor(100000 + Math.random() * 900000)}`;
      ...
      try {
        await supabase.from('users').upsert({
          ...
          cycle_earning_count: 0
        }, { onConflict: 'id' });
      } catch (insertErr: any) {
        console.warn('Upsert users notice:', insertErr.message);
      }
